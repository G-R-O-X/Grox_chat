...
3. **校验平面**：`Cat`、`Dog`、`Tron`、`Writer` 在每轮末尾进行奖励、惩戒、守法裁决和事实沉淀。
...
flowchart TB
    ...
    Validators[Cat / Dog / Tron / Writer]
...
`Fact` 是经过 Writer 或系统验证后可复用的知识对象。它必须具备：
...
Writer 是元评论家与事实加工器。它不负责主辩论，而负责：
...
## 13. Writer 到 Fact 的知识沉淀

Writer 不是只发评论，它还负责把可确认知识沉淀下来。
...
这一步是整个系统形成长期知识增益的关键。如果没有 Writer 入库，RAG 就只能反复检索旧材料，无法随着讨论变聪明。
...
Writer 除了评论正文，还必须能明确标注可入库事实，便于系统抽取。
