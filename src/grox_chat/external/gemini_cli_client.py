...
async def _discover_project_id() -> str:
    ...
    async with session.post(
        ...
        if resp.status != 200:
            ...
            raise RuntimeError(f"loadCodeAssist failed ({resp.status}): {error_body[:500]}")
        data = await resp.json()

    project_id = data.get("cloudaicompanionProject")
    if not project_id:
        raise RuntimeError("Could not discover project ID from loadCodeAssist.")
    ...
    for current_model in models_to_try:
        try:
            ...
            if is_retryable and enable_fallback:
                logger.warning(f"[GeminiCLI] {current_model} failed ({error_str[:100]}), trying next model...")
                last_error = e
                continue
            raise
