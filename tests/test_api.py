...
async def test_persist_message_embeds_standard_messages():
    with patch("agent_chatroom.api.aget_embedding", new=AsyncMock(return_value=[0.1] * 384)):
        with patch("agent_chatroom.api.insert_message_with_embedding", return_value=42) as insert_message:
            with patch("agent_chatroom.api.post_message") as post_message:
...
async def test_persist_message_falls_back_when_embedding_fails():
    with patch("agent_chatroom.api.aget_embedding", new=AsyncMock(return_value=None)):
        with patch("agent_chatroom.api.insert_message_with_embedding") as insert_message:
            with patch("agent_chatroom.api.post_message", return_value=11) as post_message:
...
def test_search_messages_hybrid_merges_dense_and_lexical_results():
    ...
    with patch("agent_chatroom.api.search_messages", return_value=dense):
        with patch("agent_chatroom.api.search_messages_lexical", return_value=lexical):
...
def test_search_facts_hybrid_merges_lexical_hits_before_truncating():
    ...
    with patch("agent_chatroom.api.search_facts", return_value=dense):
        with patch("agent_chatroom.api.search_facts_lexical", return_value=lexical):
