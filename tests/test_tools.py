...
from agent_chatroom.tools import react_search_loop
...
async def test_react_search_loop_skips_search_when_not_needed():
    ...
    with patch("agent_chatroom.tools.query_minimax", new=mock_query):
        with patch("agent_chatroom.tools.minimax_search", new=AsyncMock()) as mock_search:
...
async def test_react_search_loop_runs_one_search_then_answers():
    ...
    with patch("agent_chatroom.tools.query_minimax", new=mock_query):
        with patch("agent_chatroom.tools.minimax_search", new=mock_search):
            result, search_failed = await react_search_loop("agent1", "prompt", max_iter=2)
...
async def test_react_search_loop_preserves_original_prompt_for_final_answer_after_search():
    ...
    with patch("agent_chatroom.tools.query_minimax", new=fake_query_minimax):
        with patch("agent_chatroom.tools.minimax_search", new=mock_search):
