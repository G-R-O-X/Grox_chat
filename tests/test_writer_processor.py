...
async def test_process_writer_output_respects_explicit_empty_facts():
    with patch("grox_chat.writer_processor.api.fact_exists", return_value=False):
        with patch("grox_chat.writer_processor.aget_embedding", new=AsyncMock()) as embedding:
            with patch("grox_chat.writer_processor.api.insert_fact_with_embedding") as insert_fact:
...
async def test_process_writer_output_deduplicates_duplicate_structured_facts():
    with patch("grox_chat.writer_processor.api.fact_exists", return_value=False):
        with patch("grox_chat.writer_processor.aget_embedding", new=AsyncMock(return_value=[0.1] * 384)) as embedding:
            with patch("grox_chat.writer_processor.api.insert_fact_with_embedding", return_value=5) as insert_fact:
                await process_writer_output(
                    1,
                    "unused",
...
async def test_process_writer_output_ignores_non_string_fact_entries():
    with patch("grox_chat.writer_processor.api.fact_exists", return_value=False):
        with patch("grox_chat.writer_processor.aget_embedding", new=AsyncMock(return_value=[0.1] * 384)) as embedding:
            with patch("grox_chat.writer_processor.api.insert_fact_with_embedding", return_value=7) as insert_fact:
                await process_writer_output(
                    1,
                    "unused",
